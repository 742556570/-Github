...
ClBnkCrd clBnkCrdObj = (ClBnkCrd)CacheHelper.getObject(AppCommonConstants.getValStr("CACHESCHEMA"), "bankcard_key"+policyNo);
    ...
    String mnthBJRate = premium.multiply(new BigDecimal("100")).toString()+"%";
    String mnthRate = monthPre.divide(numberOfMoney).setScale(6,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal("100")).toString()+"%";
		...
		<table class="table_set">
			<tr class="tr_set">
				<td class="td_common font10" colspan="5">
					<b>阳光财产保险股份有限公司<br/>个人借款保证保险 电子保单信息确认</b>
				</td>
			</tr>

			<tr class="tr_set">
				<td class="td_common" colspan="5" style="text-align: right;">保单号：以保险单内容为准</td>
			</tr>
			...
			<tr class="tr_set">
				<td rowspan="3"><b>保险费</b></td>
				<td class="td_set15">总保险费</td>
				<td class="td_common" colspan="3">人民币（大写） <%=sumPreAmt%> ￥ <%=sumPre %></td>
			</tr>

			<tr class="tr_set">
				<td class="td_set15">每月保险费金额</td>
				<td class="td_common" colspan="3">人民币（大写）<%=monthPreAmt%> ￥  <%=monthPre %></td>
			</tr>
			...
			<%-- <tr class="tr_set">
				<td class="td_set15"><b>贷款合同编号</b><%=applyObj.getPolicyNo() %></td>
				<td class="td_common" colspan="4"></td>
			</tr> --%>
			...
			<tr class="tr_set">
				<td colspan="5" class="td_common" style="text-align: left;">
					<p><b>投保人声明：</b></p>
					<p class="indent"><b>本人已知悉、理解、认可并仔细核对了本保险单信息确认的全部内容，愿意按照本保险单信息确认页为基础而生成的保险单的约定投保本保险，没有异议。本人认可，如果保险合同中《阳光财产保险股份有限公司个人借款保证保险（一年期）（2016版）投保单》的内容与《阳光财产保险股份有限公司个人借款保证保险（一年期）（2016版）保险单》的内容有所出入，以保险单内容为准。本人已详细阅读所附保险条款，尤其是关于免除保险人责任的条款、投保人及被保险人义务的规定。</b></p>
					<p><b>特别提示：</b></p>
					<div>
			...
			<tr class="tr_set">
				<td class="td_common" colspan="5" style="text-align: left;">
					<div>
						...
						<p style="float: left; width: 50%">签单日期：<span id="signDate"><%=yyyy %> - <%=MM %> -  <%=dd %></span></p>
					</div>
					<div style="clear: both;"></div>
					<div>
			...
			<%-- <tr class="tr_set">
				<td class="td_common" colspan="5" style="text-align: left;">
					<div>
						<p style="float: left; width: 50%">保险人名称：<%=idCardInfoObj.getCUST_NAME() %></p>
						<p style="float: left; width: 50%">签单日期：<%=yyyy %> - <%=MM %> -  <%=dd %></p>
					</div>
					<div style="clear: both;"></div>
					<div>
						<p style="float: left; width: 50%">保险人地址：<%=idCardInfoObj.getLIVE_ADDR() %></p>
						<p style="float: left; width: 50%">注：保险条款附后。</p>
					</div>
					<div style="clear: both;"></div>
