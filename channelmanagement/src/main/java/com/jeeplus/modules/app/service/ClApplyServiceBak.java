...
@Service
public class ClApplyServiceBak {

	private final static Logger logger = LoggerFactory.getLogger(ClApplyServiceBak.class);
	...
	public void realApply(ClUsr usr, String policyNo,DecisionReq od) throws Exception {
		...
		ucService.fadadaContract(idCard.getCUST_NAME(), idCard.getID_NO(), usr.getUSR_TEL(), paramsHt, "YGSDPH80000006-1", "个人征信授权书（阳光）--央行", "1", "签章处", "", usr.getUSR_CDE(), "3-3", "","CI06");

		paramsHt =  net.sf.json.JSONObject.fromObject(fpService.getParams(usrCde, "YGSDPH80000007-1", policyNo,date));
		ucService.fadadaContract(idCard.getCUST_NAME(), idCard.getID_NO(), usr.getUSR_TEL(), paramsHt, "YGSDPH80000007-1", "个人信息查询使用授权书", "1", "签章处", "", usr.getUSR_CDE(), "3-4", "","CI06");
		
		paramsHt =  net.sf.json.JSONObject.fromObject(fpService.getParams(usrCde, "YGSDPH80000009-1", policyNo,date));
		ucService.fadadaContract(idCard.getCUST_NAME(), idCard.getID_NO(), usr.getUSR_TEL(), paramsHt, "YGSDPH80000009-1", "贷款协议（信用类）信托-阳光-附件一个人信息授权书", "1", "签章处", policyNo, usr.getUSR_CDE(), "4-2", "","CI08");
	...
	//	end = new Date().getTime();
		...
		if (false/*"error".equals(result.getString("code"))*/) {
		...
		if (/*"ok".equals(result.getString("code"))*/true) {
			int j = 0;
			if ( /*result.getJSONObject("data").isEmpty()*/false) {//
...
//				if (clPreDetail == null) {
//					msg = "ClPremiumDetail is null for usr : " + usr.getUSR_CDE() + "=" + usr.getUSR_TEL();
...
//				String clientFlag = ipolicyResult.getString("clientFlag");
				if (/*!clientFlag.equals("success")*/false) {
					msg += "保单号为:" + policyNo + "的订单无效保单失败。";
//					addFailedFirst(usr, policyNo, "InvaliPolicy");
//					apiInfo = new ClApiInfo(usr.getUSR_CDE(), usr.getUSR_TEL(), AppNormalConstants.INVALID_POLICY, AppNormalConstants.INVALID_POLICY, DateUtils.getNowFullFmt(), (int)(end-start), invaliPolicyXml, "FAIL", ipolicyResult.toJSONString());
//					apiInfoDao.insert(apiInfo);
					return;
				}
//				apiInfo = new ClApiInfo(usr.getUSR_CDE(), usr.getUSR_TEL(), AppNormalConstants.INVALID_POLICY, AppNormalConstants.INVALID_POLICY, DateUtils.getNowFullFmt(), (int)(end-start), invaliPolicyXml, "SUCCESS", ipolicyResult.toJSONString());
//				apiInfoDao.insert(apiInfo);
...
//				 apply, usr, usrInfo, idCard, usrCnts, result.getJSONObject("data"),premium,crdtExt);
...
//				 String pmXml2 = new PMXmlStringUtils().getPMRequestXML("2",capChannel, prdInfo,
//						 apply, usr, usrInfo, idCard, usrCnts, result.getJSONObject("data"),premium,crdtExt);
...
//				 String detail = rJsonObject2.get("detail").toString();
				if (/*system_decision.equals("A")*/true) {
					j = usrLmtAmtService.updateUsedAmt(usr.getUSR_CDE(), new BigDecimal(apply.getApplyAmt()));
					System.out.println("");
//					apiInfo = new ClApiInfo(usr.getUSR_CDE(), usr.getUSR_TEL(), AppNormalConstants.PM, AppNormalConstants.PM_DECISION_SERVICE, DateUtils.getNowFullFmt(), (int)(end-start), pmXml2, "SUCCESS", pmResult2);
//					apiInfoDao.insert(apiInfo);
				} else {
//					apiInfo = new ClApiInfo(usr.getUSR_CDE(), usr.getUSR_TEL(), AppNormalConstants.PM, AppNormalConstants.PM_DECISION_SERVICE, DateUtils.getNowFullFmt(), (int)(end-start), pmXml2, "FAIL", pmResult2);
//					apiInfoDao.insert(apiInfo);
					apply.setStCde("10");
...
//				boolean loanApply = loanApplyReq(capChannel, apply, prdInfo, idCard, usr, usrInfo, bankcard, usrCnts,premium);
//				if (!loanApply) {
...
//				}
				...
				apply.setStCde("12");
				 apply.setTrdResult("A");
				 apply.setTrdDetail("GK"+":"+"R1");
				 apply.setMdfDt(DateUtils.getNowFullFmt());
				 apply.setCreditNo(crdtExt.getUSR_APPSEQ());
				...
				if(isDelaySendSms) {
					...
					sendResult = new SmsCollectionUtil().sendSmsByTime(usr.getUSR_TEL(), AppNormalConstants.LOAN, conten_params,time);//true;
				} else {
...
//				apiInfo = new ClApiInfo("", usr.getUSR_TEL(), AppNormalConstants.SMS_API_NAME, AppNormalConstants.SMS_API_SERVICE_NAME, DateUtils.getNowFullFmt(), (int)(e-s), sms.getContentByConditonAndParams(AppNormalConstants.LOAN, conten_params), status, new Boolean(sendResult).toString());
//				apiInfoDao.insert(apiInfo);
				String dt1  = DateUtils.formatDate(new Date(), "yyyy-MM-dd");
				paramsHt =  net.sf.json.JSONObject.fromObject(fpService.getParams(usrCde, "YGSDPH80000014-1", policyNo,dt1));
				...
				paramsHt =  net.sf.json.JSONObject.fromObject(fpService.getParams(usrCde, "YGSDPH80000008-1", policyNo,dt1));
				...
				paramsHt =  net.sf.json.JSONObject.fromObject(fpService.getParams(usrCde, "YGSDPH80000010-1", policyNo,dt1));
				ucService.fadadaContract(idCard.getCUST_NAME(), idCard.getID_NO(), usr.getUSR_TEL(), paramsHt, "YGSDPH80000010-1", "保险单", "2", "投保人签名", policyNo, usr.getUSR_CDE(), "5-1", "核保","CI12");

				paramsHt =  net.sf.json.JSONObject.fromObject(fpService.getParams(usrCde, "YGSDPH80000012-1", policyNo,dt1));
				...
				paramsHt =  net.sf.json.JSONObject.fromObject(fpService.getParams(usrCde, "YGSDPH80000013-1", policyNo,dt1));
	...
	private boolean loanApplyReq(ClCapChannel channel, ClUsrApply apply, ClPrdInfo prd, ClIdCardInfo idcard, ClUsr usr,
		...
		try {
			...
			creditLoanApplyVO.setFAMILYADD(
					usrinfo.getLIVE_PROVINCE() + usrinfo.getLIVE_CITY() + usrinfo.getLIVE_AREA() + usrinfo.getLIVE_ADDR());// 北京
			...
			if (reulst.contains("数据保存成功")) {
				ClApiInfo apiInfo = new ClApiInfo(usr.getUSR_CDE(), usr.getUSR_TEL(), AppNormalConstants.ZHANGWU, AppNormalConstants.FANGKUAN, DateUtils.getNowFullFmt(), (int)(end-start), xmlStr, "SUCCESS", reulst);
				apiInfoDao.insert(apiInfo);
				return true;
			}
			ClApiInfo apiInfo = new ClApiInfo(usr.getUSR_CDE(), usr.getUSR_TEL(), AppNormalConstants.ZHANGWU, AppNormalConstants.FANGKUAN, DateUtils.getNowFullFmt(), (int)(end-start), xmlStr, "FAIL", reulst);
			apiInfoDao.insert(apiInfo);
			return false;
		} catch (Exception e) {
	...
	// 同步放款信息
	private boolean syncLoanMsg(ClUsr usr ,ClUsrApply apply, ClCapChannel channel, ClPrdInfo prd, ClIdCardInfo idcard,BigDecimal premium) {
		try {
			...
			if ("success".equals(slmc.getErrorMsg())) {
				ClApiInfo apiInfo = new ClApiInfo(usr.getUSR_CDE(), usr.getUSR_TEL(), AppNormalConstants.HESUAN, AppNormalConstants.SYNC_LOANMSG, DateUtils.getNowFullFmt(), (int)(end-start), JSONObject.toJSONString(reqLoanMessageVO), "SUCCESS", JSONObject.toJSONString(slmc));
				apiInfoDao.insert(apiInfo);
				return true;
			}
			ClApiInfo apiInfo = new ClApiInfo(usr.getUSR_CDE(), usr.getUSR_TEL(), AppNormalConstants.HESUAN, AppNormalConstants.SYNC_LOANMSG, DateUtils.getNowFullFmt(), (int)(end-start), JSONObject.toJSONString(reqLoanMessageVO), "FAIL", JSONObject.toJSONString(slmc));
			apiInfoDao.insert(apiInfo);
			return false;
		} catch (Exception e) {
			// TODO: handle exception
...
public void testPMBak(ClUsr usr, String policyNo,DecisionReq od) throws Exception {
...
//		result.put("code", "ok");
		...
		if ("ok".equals(result.getString("code"))) {
			int j = 0;
			if ( false) {//result.getJSONObject("data").isEmpty()
				msg += "保单号为:" + policyNo + "的订单未能获取衍生数据。";
				addFailedFirst(usr, policyNo, "DerivedData");
				return;
			} else {
				...
				BigDecimal premium = clPreDetail.getPREMIUM_RATE();
				ClCrdtExt crdtExt = crdtDao.getClCrdtExtByUsrCde(usr.getUSR_CDE());
//				prdInfo.setPREMIUM_RATE(premium);
...
//				}
				...
				// PM
				String pmXml = new PMXmlStringUtils().getPMRequestXML("1", capChannel, prdInfo, apply, usr, usrInfo,
				...
				String pmXml2 = new PMXmlStringUtils().getPMRequestXML("2", capChannel, prdInfo, apply, usr, usrInfo,
						idCard, usrCnts, result.getJSONObject("data"), premium, crdtExt);
				System.out.println(pmXml2);
				String pmResult2 = pmapi.preCredExt(pmXml2);
				JSONObject rJsonObject2 = new PMXMlResultUtils().parseDecisionResultXML(pmResult2);
				String default_reason = rJsonObject2.getString("default_reason");
				String system_decision = rJsonObject2.getString("system_decision");
...
//				}
				// 同步放款信息
//				if (!syncLoanMsg(apply, capChannel, prdInfo, idCard,premium)) {
//					msg += "保单号为:" + policyNo + "的订单申请放款失败。";
//					addFailedFirst(usr, policyNo, "SyncLoanMsg");
//					return;
//				}
...
//				apply.setStCde("12");
//				 apply.setTrdResult(system_decision);
//				 apply.setTrdDetail(default_decision2);
//				apply.setTrdResult("A");
//				apply.setTrdDetail("通过");
//				CacheHelper.setObject(AppCommonConstants.getValStr("CACHESCHEMA"), "apply_key", apply,
//						AppCommonConstants.getValInt("CACHEVALIAPPLYTTL"));
//				Object obj = CacheHelper.getObject(AppCommonConstants.getValStr("CACHESCHEMA"), "apply_key");
//				System.out.println("缓存值" + obj.toString());
