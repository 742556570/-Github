...
public class XMLFileUtil{
    ...
    */
	 ...
	 * @throws Exception
	 */
	public Map readModuleServiceFromXMLFile(String dir) throws Exception{
		...
		for(int k=0; k<al.size(); k++){
			...
			NodeList buildingList = doc.getElementsByTagName(BUILDINGELEMENT);
			Element ele = (Element)buildingList.item(0);
			String moduleName = ele.getAttribute("moduleName");//模块名称
			...
			for(int s=0; s<buildingList.getLength(); s++){
				...
				//得到这个构建下所有的moduleService
				NodeList moduleServiceList = buildElement.getElementsByTagName(MODULESERVICE);
				/*
				 * 取出每个moduleService节点信息，存入一个map
				 */
				for(int i=0; i<moduleServiceList.getLength(); i++){
					//创建单个moduleService信息存储容器
					HashMap<String, String> serviceModuleMap = new HashMap<String, String>();
					
					Node moduleServiceNode = moduleServiceList.item(i);
					Element moduleServiceElement  = (Element)moduleServiceNode;
					String serviceId = moduleServiceElement.getAttribute(ID);
					String serviceDescribe = moduleServiceElement.getAttribute(DESCRIBE);
					String serviceComproperty = moduleServiceElement.getAttribute(COMPROPERTY);
					
					serviceModuleMap.put(MODULENAME, moduleName);
					serviceModuleMap.put(DESCRIBE, serviceDescribe);
					serviceModuleMap.put(COMPROPERTY, serviceComproperty);
					
					NodeList classnameList = moduleServiceElement.getChildNodes();
					String classname = ((Node)classnameList.item(0)).getNodeValue().trim();
					serviceModuleMap.put(CLASSNAME, classname);
					
					infoMap.put(serviceId, serviceModuleMap);
				}
	 ...
	 */
	public Map<String, String> loadModuleInitializerCfgFiles(String dir) throws Exception{
		...
		for (int i = 0; i < fileList.size(); i++) {
			...
			for(int s=0; s<buildingList.getLength(); s++){
				...
				for(int k=0; k<initializerList.getLength(); k++){
					...
					retMap.put(className, moduleName);
				}
			}
		}
		return retMap;
