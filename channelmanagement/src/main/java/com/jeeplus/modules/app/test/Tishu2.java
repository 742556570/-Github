...
public class Tishu2 {
	...
	public static void main(String[] args) throws Exception {
		List<String> f1_lines = FileUtils.readLines(new File("d:/f1.txt"));
		
		
		List<String> f2_lines = FileUtils.readLines(new File("d:/f2.txt"));
		...
		for (String f2_line : f2_lines) {
			...
			String pn = "";
			if(tmpList.size()>0) {
				String tmp1 = tmpList.get(0);
				if(!StringUtils.isEmpty(tmp1)) {
					pn = tmp1.split("\t")[0];
				}
			}
			String result = pn+"\t"+cols[2] + "\t" + cols[1] + "\t" + cols[0] + "\t" + "-" + "\t" + cols[3] + "\t-\t-";
			tmpList.add(result);
		}
		...
		File finalfile = new File("d:/final.txt");
		if(finalfile.exists()) {
			finalfile.delete();
		}
